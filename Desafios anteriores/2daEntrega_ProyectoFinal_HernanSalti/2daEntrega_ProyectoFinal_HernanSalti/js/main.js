...
class Tienda {
    ...
    renderCarrito = () => document.getElementById("carrito").innerHTML = this.carrito.map(item => item.cardCarrito()).join("");
    
    renderStock = () => document.getElementById("productos").innerHTML = this.stock.map(item => item.card()).join("<br/>");
    
    calcularTotal =() => {
        this.total = this.carrito.reduce((a,b) => a + b.precio , 0);
        document.getElementById("destino").innerHTML = "El total de su compra es: $"+ this.total ;
    };
}
...
[...document.getElementsByClassName("botonesAniadir")].forEach(boton => boton.onclick = agregarAlCarrito);
